...
void enviarAccionBackend(String tipoAccion, int userId, int actionId = 0);
...
void loop() {
  ...
  // ✅ CORRECCIÓN: Siempre procesar NFC para acceso normal
  procesarNFC();
  ...
  } else if (!esperandoTarjetaRegistro) {
    // Solo procesar teclado para login si no estamos en modo registro
    procesarTecladoAcceso();
  }

  // El registro de tarjeta se maneja dentro de procesarNFC()
}
...
void procesarMensajeWebSocket(uint8_t* payload, size_t length) {
  ...
  // 🆕 NUEVO: REGISTRO DE TARJETA NFC DESDE BACKEND
    ...
    Serial.println("[NFC] 📝 Modo registro activado para usuario ID: " + String(usuarioRegistroId));
    
    // Confirmar recepción al backend
    webSocket.sendTXT("{\"type\":\"nfc_registration_received\",\"user_id\":" + String(usuarioRegistroId) + "}");
  }
  ...
  // Acciones desde el backend
    ...
    if (actionType == "DOOR_OPEN" || command == "DOOR_OPEN") {
      ...
      enviarAccionBackend("DOOR_OPEN", userId, actionId);
      ...
      enviarAccionBackend("GARAGE_OPEN", userId, actionId);
...
// ✅ NUEVA FUNCIÓN: Procesar teclado para acceso normal
void procesarTecladoAcceso() {
...
// ✅ FUNCIÓN NFC MEJORADA: Maneja tanto acceso normal como registro
void procesarNFC() {
  ...
  if (tiempoActual - ultimaLectura >= intervaloLectura) {
    ...
    // ✅ DECISIÓN: ¿Es para registro o para acceso?
...
// ================== REGISTRO DE TARJETA NFC CORREGIDO ==================
void registrarTarjetaEnBackend(String uid) {
  if (WiFi.status() != WL_CONNECTED) {
    mostrarMensajeLCD("ERROR CONEXION", "Sin WiFi", 2000);
  ...
  // ✅ CORRECCIÓN: Usar siempre autenticación para registrar tarjetas
  if (tokenActual != "") {
    http.addHeader("Authorization", "Bearer " + tokenActual);
    Serial.println("[NFC-REG] 🔐 Registrando con token de autenticación");
  } else {
    Serial.println("[NFC-REG] ⚠️ No hay token, intentando sin autenticación");
  }
  ...
  Serial.println("[NFC-REG] Registrando tarjeta: " + requestBody);
  
  int httpCode = http.POST(requestBody);
  Serial.printf("[NFC-REG] HTTP Code registro: %d\n", httpCode);
  
  if (httpCode == 200 || httpCode == 201) {
    mostrarMensajeLCD("TARJETA REGISTRADA", "EXITOSAMENTE", 3000);
    Serial.println("✅ Tarjeta registrada exitosamente");
    
    // Notificar al backend via WebSocket
    webSocket.sendTXT("{\"type\":\"nfc_card_registered\",\"card_uid\":\"" + uid + "\",\"user_id\":" + String(usuarioRegistroId) + ",\"card_name\":\"" + nombreTarjetaRegistro + "\"}");
    
  } else {
    String response = http.getString();
    Serial.println("[NFC-REG] Error response: " + response);
    
    if (httpCode == 401) {
      mostrarMensajeLCD("ERROR AUTENTICACION", "Token invalido", 3000);
      Serial.println("❌ Error 401 - No autorizado. Token inválido o expirado.");
    } else if (httpCode == 400) {
      mostrarMensajeLCD("TARJETA YA EXISTE", "Use otra tarjeta", 3000);
      Serial.println("❌ Error 400 - Tarjeta ya registrada.");
    } else {
      mostrarMensajeLCD("ERROR REGISTRO", "Cod: " + String(httpCode), 3000);
...
// ✅ CORRECCIÓN: Función enviarAccionBackend actualizada para enviar id_action
void enviarAccionBackend(String tipoAccion, int userId, int actionId) {
  if (WiFi.status() != WL_CONNECTED) {
  ...
  // ✅ CORRECCIÓN: Incluir id_action si está disponible
  if (actionId > 0) {
    doc["id_action"] = actionId;
    Serial.println("[ACTION] 📋 Incluyendo id_action: " + String(actionId));
  }
...
void abrirPuertaPrincipal() {
  ...
  enviarNotificacionAcceso("DOOR_OPEN");
  enviarAccionBackend("DOOR_OPEN", userId, 0);
...
void abrirGaraje() {
  ...
  // Luego enviar las notificaciones
  enviarNotificacionAcceso("GARAGE_OPEN");
  enviarAccionBackend("GARAGE_OPEN", userId, 0);
